import { initGame, parseRack, linkifyDef, backBtn, tileValues, letterScore, dailyStudySlice, dailyStudyText, studyListText, studyDateLabel, STUDY_TWOS, STUDY_THREES, lexicalDefinition, defBody, extractFormOf, isInflectionDef } from './game.js?v=109'
import { loadHistory, rememberWord, mergeHistory, historyLabel, historyDayLabel, clearHistory } from './history.js?v=109'
import { loadFavorites, toggleFavorite, favButtonHtml, paintFavStar } from './favorites.js?v=109'
import { isCompetitive, isKids, isTraining, setGameMode, initGoogleSignIn, checkSession, handleGoogleCallback, logout, getCurrentUser, fetchDailyTrail, fetchLeaderboard, getTrailData } from './competitive.js?v=109'
import { initLang, setLang, setDict, getLang, getDict, dictSpec, dictLabel, t, DICTS } from './i18n.js?v=109'
...
function recordWords(entries) {
  ...
  if (getCurrentUser()) {
    import('./competitive.js?v=109').then(({ saveHistoryWord }) => {
...
async function syncCloudHistory() {
  ...
  try {
    const { fetchHistory } = await import('./competitive.js?v=109')
    ...
    const { saveHistoryWord } = await import('./competitive.js?v=109')
...
document.getElementById('hist-clear')?.addEventListener('click', async () => {
  ...
  if (getCurrentUser()) {
    try {
      const { clearCloudHistory } = await import('./competitive.js?v=109')
