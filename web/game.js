import { t, getLang, getDict, dictLabel } from './i18n.js?v=121'
import { favButtonHtml, paintFavStar } from './favorites.js?v=121'
...
export function initGame({ ask, tilesHtml, escapeHtml, normalize, ready, define, isCompetitive, isKids, isTraining, onDeal, onPlayed }) {
  ...
  async function syncRankedScore(percent, word, context) {
    ...
    const promise = (async () => {
      const { submitCompete, fetchLeaderboard, getCurrentUser, getTrailData, competeAccepted } =
        await import('./competitive.js?v=121')
  ...
  async function initRanked(kids, requestId = modeSeq) {
    const { initGoogleSignIn, checkSession, getCurrentUser, handleGoogleCallback, fetchDailyTrail, fetchLeaderboard } = await import('./competitive.js?v=121')
  ...
  async function loadGeneralBoard(kids) {
    const { fetchLeaderboard } = await import('./competitive.js?v=121')
