...
final class Defs {
    ...
    private static final Pattern[] FORM_OF = {
        ...
        Pattern.compile("(?:indicatif|subjonctif|conditionnel)(?: présent| passé| imparfait)? de\\s+" + WORD, FLAGS),
        ...
        Pattern.compile("participe (?:pass[eé]|pr[eé]sent)(?:[^.]{0,40}?)(?:du verbe|de)\\s+" + WORD, FLAGS),
        Pattern.compile("variante(?: orthographique)? de\\s+" + WORD, FLAGS),
        Pattern.compile("forme(?:s| conjuguée)? de\\s+" + WORD, FLAGS),
        Pattern.compile("inflection of\\s+" + WORD, FLAGS),
        Pattern.compile("alternative form of\\s+" + WORD, FLAGS),
