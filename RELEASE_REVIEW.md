...
| Area | Evidence | Limit |
| --- | --- | --- |
| Integrated Node suite | 218 tests passed, 0 failed on Windows and again in the synchronized Mac checkout. Security, persistence, activity, session ordering, dictionaries, service worker, release assets and sync are covered. | Fixtures do not exercise real Google accounts, production writes or distributed abuse. |
...
| Shared analytics integration | 16 checks passed both in isolation and on the Mac. The deployed injector honors auth-page opt-out and starts Verimots analytics consent denied. | Live Google sign-in and authenticated cookie behavior need device/account acceptance. |
| Public deployment | 59 successful GET checks after restart, including all 45 service-worker shell URLs, expected MIME types, matching JS/CSS/dictionary bytes, four-language trail/board APIs, auth-page analytics exclusion and denied-by-default consent on both hosts. | Anonymous read-only checks; no production account, mail or score mutations. Public Android metadata remains 4.7.0. |
...
4. Complete authenticated acceptance on the deployed origins: login/logout, owner-safe history, installed offline behavior and upgrades from the previous service worker. Anonymous public checks cannot prove these account and update flows.
...
- Candidate code commit: `fb5ddf4b0400bc1aa19fd2d0980772cbf662620e`, branch `release/polish-20260916` on `pfaisant/verimots`.
- Mac source: `/Users/clawdbot/Dev/verimots`. Original dirty source verified against all 234 snapshot files and preserved in stash `ce3384260fd8988bc8fad8600b6b7143d683f14f` before switching to the candidate branch.
- Deployment: `/Users/clawdbot/Dev/AiConglomerate`. All 276 baseline hashes matched before deployment. Applied 55 mapped file changes; the final sync plan reports zero differences. Unrelated files and public APK/AAB downloads were preserved.
- Rollback copies and applied plan: `/Users/clawdbot/Dev/verimots-backups/release-20260916/deployment`. Shared analytics rollback and patch: adjacent `shared-integration/`. The [integration patch](integration/ga-inject.patch) is also tracked here for future host updates.
- Restarted `system/com.aiconglomerate.serve` through its owned process/KeepAlive service and `gui/501/com.aiconglomerate.analytics-proxy`. The separate server on port 4321 was not touched.
- Signed, unpublished APK/AAB and hash manifest: `D:/_agent_work/releases/verimots-4.7.1-82/` and `/Users/clawdbot/Dev/verimots-releases/4.7.1-82/`. Both copies have matching SHA-256 hashes; the isolated Mac build remains `/tmp/verimots-android-review-20260916`.
