...
test('the app mounts leaderboard rows directly and escapes public player names', async t => {
  ...
  assert.equal(ui.requests[0].url, '/api/game/board?category=combined&scope=7d&lang=any')
...
test('changing filters keeps app navigation and ignores an older response', async t => {
  ...
  ui.change('scope', 'all')
  assert.match(ui.requests[2].url, /category=bingo&scope=all&lang=any/)
  ui.reply(2)
  await flush()
...
test('language filter defaults to all and retains category and period without changing UI language', async t => {
  const ui = setup(t, {lang:'en',standalone:true})
  assert.equal(ui.select('board').value,'any')
  assert.equal(ui.select('category').value,'combined')
  assert.equal(ui.select('scope').value,'7d')
  assert.match(ui.select('board').innerHTML,/All languages/)
  for (const language of ['fr','en','es','ca']) assert.match(ui.select('board').innerHTML,new RegExp(`value="${language}"`))
  ui.reply(0)
  await ui.api.ready
  ui.change('board','ca')
  assert.match(ui.requests[1].url,/category=combined&scope=7d&lang=ca/)
  assert.match(ui.urls.at(-1),/lang=en.*board=ca/)
  ui.reply(1)
  await flush()
  ui.change('board','any')
  assert.match(ui.requests[2].url,/category=combined&scope=7d&lang=any/)
  ui.reply(2)
  await flush()
})
